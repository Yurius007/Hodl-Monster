...
function copyTestTokenAddress() {
    const address = document.getElementById('mintTokenAddressDisplay').textContent.trim();
...
function showCopySuccess() {
    ...
    const originalHTML = '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg>';
...
async function lockTokens(e) {
    ...
    try {
        ...
        showTxStatus('Tokens locked successfully! An NFT has been minted to represent your lock.', 'success');
...
async function fetchBatchTokenInfo(inputElement) {
    ...
    try {
        // Use batch endpoint for single token (works for one or many)
        const response = await fetch(`/api/${currentChain}/tokens-batch`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                tokens: [tokenAddress],
        ...
        if (result.success && result.tokens[tokenAddress]) {
            const tokenData = result.tokens[tokenAddress];
            ...
            if (tokenData.balance !== undefined) {
                const balance = formatTokenAmount(tokenData.balance, tokenData.decimals);
                info.userBalance = balance;
                info.userBalanceRaw = tokenData.balance;

                balanceSpan.textContent = `${balance} ${tokenData.symbol}`;
                balanceSpan.classList.remove('hidden');
                maxBtn.disabled = false;
            }

            batchTokensInfo[tokenAddress.toLowerCase()] = info;
            infoDiv.innerHTML = `<strong>${tokenData.symbol}</strong> - ${tokenData.name}`;
...
async function batchLockMultipleTokens(e) {
    ...
    try {
        ...
        // Updated to use unified /encode/lock endpoint
        const response = await fetch(`/api/${currentChain}/encode/lock`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
        ...
        showTxStatus(`${tokenAddresses.length} token(s) locked successfully! An NFT has been minted.`, 'success');
...
function displayAllLocks(locks, tokenCount) {
    ...
    locks.forEach(lock => {
        if (now >= lock.unlockTime) totalClaimable++;
    });

    summary.innerHTML = `
        <h3>Total Lock NFTs: ${locks.length} across ${tokenCount} token(s)</h3>
    ...
    locks.forEach(lock => {
        const isClaimable = now >= lock.unlockTime;
        const status = isClaimable ? 'claimable' : 'locked';
        const statusText = isClaimable ? 'Claimable' : 'Locked';

        // Build tokens display
        const tokensDisplay = lock.tokens.map(token => `
            <div class="lock-detail">
                <span class="lock-detail-label">${token.tokenSymbol}</span>
                <span class="lock-detail-value">${formatTokenAmount(token.amount, token.tokenDecimals)} ${token.tokenSymbol}</span>
            </div>
        `).join('');
...
async function claimLock(tokenId) {
    if (!userAddress) {
        alert('Please connect your wallet first');
        return;
    }
    ...
    try {
        ...
        showTxStatus('Tokens claimed successfully! The lock NFT has been burned.', 'success');
